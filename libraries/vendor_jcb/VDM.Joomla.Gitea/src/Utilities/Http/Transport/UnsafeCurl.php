...
namespace VDM\Joomla\Gitea\Utilities\Http\Transport;
...
/**
 * A rewritten cURL transport that ALWAYS allows self-signed certificates.
 * 
 * @since 5.1.4
 */
final class UnsafeCurl extends AbstractTransport implements TransportInterface
{
	/**
	 * The switch to allow self signed certificates (Unsafe Curl) calls
	 *
	 * @var   bool
	 * @since 5.1.4
	 */
	protected static bool $allowSelfSigned;

	/**
	 * Always supported if cURL exists.
	 *
	 * @return bool
	 * @since 5.1.4
	 */
	...
	{
		self::$allowSelfSigned ??= (bool) Helper::getParams()
			->get('allow_selfsigned_certificates', false);

		return self::$allowSelfSigned && \function_exists('curl_version');
	}
	...
	{
		// ---------------------------------------------------------------------
		// THE ENTIRE CURL REQUEST REWRITTEN TO FORCE-INSECURE MODE
		// ---------------------------------------------------------------------
		$ch = curl_init();
		...
		$options = [
			...
			// Disable CA bundle completely
			\CURLOPT_CAINFO => null,
			\CURLOPT_CAPATH => null,

			// Disable low-level OpenSSL verification
			\CURLOPT_SSL_OPTIONS =>
