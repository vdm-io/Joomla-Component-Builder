...
namespace VDM\Joomla\Componentbuilder\Compiler\Architecture\JoomlaFive\Dashboard;
...
final class View implements ViewInterface
{
	...
	/**
	 * Build the admin dashboard display markup for Joomla 5.
	 *
	...
	protected function getDashboardDisplayState(): array
	{
		return [
			...
			'wrapperTabsClass' => 'main-card',
			'wrapperClass' => 'main-card p-3 p-lg-4',
			'rowClass' => 'row g-4',
			'mainColumnClass' => 'col-12 col-xl-9',
			'sideColumnClass' => 'col-12 col-xl-3',
	...
	{
		$state['display'][] = $this->getOpenContainer($state['containerId']);
		...
		$state['display'][] = Indent::_(2) . $this->getOpenDiv($state['rowClass']);
	}
	...
	{
		$state['display'][] = $state['tab'] . $this->getOpenDiv($state['mainColumnClass']);
	...
	{
		$state['display'][] = $state['tab'] . $this->getOpenDiv($state['sideColumnClass']);
		$state['display'][] = $state['tab'] . Indent::_(1)
	...
	protected function appendCustomTabs(array &$state): void
	{
		...
		$state['display'][] = Indent::_(2) . $this->getUiTabEndTabCode();
	 ...
	 * @since  5.1.5
	 */
	protected function getOpenContainer(string $id): string
	{
		return '<div id="' . $id . '">';
	}
