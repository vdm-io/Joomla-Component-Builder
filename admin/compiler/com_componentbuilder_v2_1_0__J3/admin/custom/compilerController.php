 ...
 * Compiler Controller
 */
class ###Component###ControllerCompiler extends JControllerLegacy
{
	/**
	...
	public function compiler()
	{
		...
		if($user->authorise('core.admin', 'com_###component###'))
		{
			...
			$message	= $app->getUserState('com_###component###.message');
			if (empty($redirect_url))
			{
				$redirect_url = JRoute::_('index.php?option=com_###component###&view=compiler', false);
				...
				$app->setUserState('com_###component###.extension_name', $counter['filename']);
			} 
			else
			{
				...
				$app->setUserState('com_###component###.extension_name', '');
				// set redirect
	...
	public function installExtention()
	{
		...
		if($user->authorise('core.admin'))
		{
			...
			$model = $this->getModel('compiler');
			$app = JFactory::getApplication();
			$fileName = $app->getUserState('com_###component###.extension_name');
			if (###Component###Helper::checkString($fileName))
			{
