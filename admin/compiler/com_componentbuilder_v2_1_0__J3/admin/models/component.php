...
{    
	...
	public function getItem($pk = null)
	{
		...
		$this->iddrjs = $item->addadmin_views;
		$this->idezco = $item->addsite_views;
	...
	*/
	public function getLugadmin_views()
	{
		// Get the user object.
		...
		if ($db->getNumRows())
		{
			...
			$iddrjs = json_decode($this->iddrjs,true);
			if (ComponentbuilderHelper::checkArray($items) && isset($iddrjs) && ComponentbuilderHelper::checkArray($iddrjs))
			{
				foreach ($items as $nr => &$item)
				{
					if ($item->id && isset($iddrjs['adminview']) && ComponentbuilderHelper::checkArray($iddrjs['adminview']))
					{
						if (!in_array($item->id,$iddrjs['adminview']))
						{
							unset($items[$nr]);
							continue;
						}
	...
	* @return mixed  An array of data items on success, false on failure.
	*/
	public function getNzfsite_views()
	{
		// Get the user object.
		$user = JFactory::getUser();
		// Create a new query object.
		...
		if ($db->getNumRows())
		{
			...
			// Filter by id Repetable Field
			$idezco = json_decode($this->idezco,true);
			if (ComponentbuilderHelper::checkArray($items) && isset($idezco) && ComponentbuilderHelper::checkArray($idezco))
			{
				foreach ($items as $nr => &$item)
				{
					if ($item->id && isset($idezco['siteview']) && ComponentbuilderHelper::checkArray($idezco['siteview']))
					{
						if (!in_array($item->id,$idezco['siteview']))
						{
							unset($items[$nr]);
							continue;
						}
