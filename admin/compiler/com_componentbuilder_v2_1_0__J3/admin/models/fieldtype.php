...
{    
	...
	*/
	public function getAlofields()
	{
		...
		// Filter by typeeguk global.
		$typeeguk = $this->typeeguk;
		if (is_numeric($typeeguk ))
		{
			$query->where('a.type = ' . (int) $typeeguk );
		}
		elseif (is_string($typeeguk))
		{
			$query->where('a.type = ' . $db->quote($typeeguk));
		}
		else
		{
		...
		if ($db->getNumRows())
		{
			...
			if (ComponentbuilderHelper::checkArray($items))
			{
				...
				{
					// convert datatype
					$item->datatype = $this->selectionTranslationAlofields($item->datatype, 'datatype');
					// convert indexes
					$item->indexes = $this->selectionTranslationAlofields($item->indexes, 'indexes');
					// convert null_switch
					$item->null_switch = $this->selectionTranslationAlofields($item->null_switch, 'null_switch');
					// convert store
					$item->store = $this->selectionTranslationAlofields($item->store, 'store');
				}
			}
	...
	*/
	public function selectionTranslationAlofields($value,$name)
	{
