...
{    
	...
	public function getBbhfields()
	{
		...
		if ($db->getNumRows())
		{
			...
			if (ComponentbuilderHelper::checkArray($items))
			{
				...
				{
					// convert datatype
					$item->datatype = $this->selectionTranslationBbhfields($item->datatype, 'datatype');
					// convert indexes
					$item->indexes = $this->selectionTranslationBbhfields($item->indexes, 'indexes');
					// convert null_switch
					$item->null_switch = $this->selectionTranslationBbhfields($item->null_switch, 'null_switch');
					// convert store
					$item->store = $this->selectionTranslationBbhfields($item->store, 'store');
				}
			}


			// Filter by id Repetable Field
			$idgayn = json_decode($this->idgayn,true);
			if (ComponentbuilderHelper::checkArray($items) && isset($idgayn) && ComponentbuilderHelper::checkArray($idgayn))
			{
				foreach ($items as $nr => &$item)
				{
					if ($item->id && isset($idgayn['field']) && ComponentbuilderHelper::checkArray($idgayn['field']))
					{
						if (!in_array($item->id,$idgayn['field']))
						{
							unset($items[$nr]);
							continue;
						}
	...
	*/
	public function selectionTranslationBbhfields($value,$name)
	{
