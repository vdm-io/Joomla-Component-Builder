...
class FieldtypesController extends AdminController
{
	...
	public function resetPowers()
	{
		...
		if($user->authorise('fieldtype.reset', 'com_componentbuilder'))
		{
			...
			try {
				FieldtypeFactory::_('Package.Builder.Get')->reset('fieldtype', $guids);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_RESET_OF_THIS_JOOMLA_FIELD_TYPES_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_RESET_FAILED') . '</h1>';
	...
	public function pushPowers()
	{
		...
		if($user->authorise('fieldtype.push', 'com_componentbuilder'))
		{
			...
			try {
				FieldtypeFactory::_('Package.Builder.Set')->items('fieldtype', $guids);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_PUSH_OF_THIS_JOOMLA_FIELD_TYPES_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PUSH_FAILED') . '</h1>';
