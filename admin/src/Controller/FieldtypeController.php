...
class FieldtypeController extends FormController
{
	...
	public function resetPowers()
	{
		...
		{
			try {
				FieldtypeFactory::_('Package.Builder.Get')->reset('fieldtype', [$guid]);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_RESET_OF_THIS_JOOMLA_FIELD_TYPE_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_RESET_FAILED') . '</h1>';
	...
	public function pushPowers()
	{
		...
		{
			try {
				FieldtypeFactory::_('Package.Builder.Set')->items('fieldtype', [$guid]);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_PUSH_OF_THIS_JOOMLA_FIELD_TYPE_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PUSH_FAILED') . '</h1>';
