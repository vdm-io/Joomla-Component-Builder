...
class Validation_ruleController extends FormController
{
	...
	/**
	 * Resets the specified Validation Rule.
	 *
	 ...
	 * 3. It checks whether the current user has the necessary permissions to reset the Validation Rule.
	 * 4. It validates the presence of the necessary item identifiers (ID and GUID).
	 * 5. If the user is authorized and the identifiers are valid, it attempts to reset the specified Validation Rule.
	...
	public function resetPowers()
	{
		...
		$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_YOU_DO_NOT_HAVE_PERMISSION_TO_RESET_THIS_VALIDATION_RULE') . '</p>';
		$status = 'error';
		$success = false;
		$has_error = false;
		...
		if ($id === null || $guid === null)
		{
			...
			$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_YOU_MUST_FIRST_SAVE_THE_VALIDATION_RULE_BEFORE_YOU_CAN_USE_THIS_FEATURE') . '</p>';
		}
		elseif($user->authorise('validation_rule.reset', 'com_componentbuilder'))
		{
			try {
				PackageFactory::_('Package.Builder.Get')->reset('validation_rule', [$guid]);
				...
				if ($success)
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_VALIDATION_RULE_HAS_SUCCESSFULLY_BEEN_RESET') . '</p>';
					$status = 'success';
				}
				elseif ($has_error)
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_RESET_OF_THIS_VALIDATION_RULE_HAS_FAILED') . '</p>';
					$status = 'error';
				}
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_RESET_OF_THIS_VALIDATION_RULE_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_RESET_FAILED') . '</h1>';
		...
		$redirect_url = Route::_(
			'index.php?option=com_componentbuilder&view=validation_rule'
	 ...
	 /**
	 * Pushes the specified Validation Rule.
	 *
	 * This function performs several checks and operations:
	 * 1. It verifies the authenticity of the request to prevent request forgery.
	 * 2. It retrieves the item data posted by the user.
	 * 3. It checks whether the current user has the necessary permissions to push the Validation Rule.
	 * 4. It validates the presence of the necessary item identifiers (ID and GUID).
	 * 5. If the user is authorized and the identifiers are valid, it attempts to push the specified Validation Rule.
	...
	public function pushPowers()
	{
		...
		// set default error message
		$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PERMISSION_DENIED') . '</h1>';
		$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_YOU_DO_NOT_HAVE_PERMISSION_TO_PUSH_THIS_VALIDATION_RULE') . '</p>';
		$status = 'error';
		$success = false;
		$has_error = false;
		...
		elseif($user->authorise('validation_rule.push', 'com_componentbuilder'))
		{
			try {
				PackageFactory::_('Package.Builder.Set')->items('validation_rule', [$guid]);
				...
				if ($success)
				{
					// set success message
					$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_SUCCESS') . '</h1>';
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_VALIDATION_RULE_HAS_SUCCESSFULLY_BEEN_PUSHED') . '</p>';
					$status = 'success';
				}
				elseif ($has_error)
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_PUSH_OF_THIS_VALIDATION_RULE_HAS_FAILED') . '</p>';
					$status = 'error';
				}
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_PUSH_OF_THIS_VALIDATION_RULE_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PUSH_FAILED') . '</h1>';
