...
class Joomla_powersController extends AdminController
{
	...
	public function resetPowers()
	{
		...
		if($user->authorise('joomla_power.reset', 'com_componentbuilder'))
		{
			...
			try {
				JoomlaPowerFactory::_('Package.Builder.Get')->reset('joomla_power', $guids);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_RESET_OF_THIS_JOOMLA_POWERS_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_RESET_FAILED') . '</h1>';
	...
	public function pushPowers()
	{
		...
		if($user->authorise('joomla_power.push', 'com_componentbuilder'))
		{
			...
			try {
				JoomlaPowerFactory::_('Package.Builder.Set')->items('joomla_power', $guids);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_PUSH_OF_THIS_JOOMLA_POWERS_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PUSH_FAILED') . '</h1>';
