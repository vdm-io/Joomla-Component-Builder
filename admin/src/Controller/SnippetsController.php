...
class SnippetsController extends AdminController
{
	...
	public function resetPowers()
	{
		...
		if($user->authorise('snippet.reset', 'com_componentbuilder'))
		{
			...
			try {
				...
				{
					if (($messages = SnippetFactory::_('Package.Message')->get($message_key, null)) !== null)
					{
	...
	public function pullPowers()
	{
		...
		// set default error message
		$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PERMISSION_DENIED') . '</h1>';
		$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_YOU_DO_NOT_HAVE_PERMISSION_TO_PULL_SNIPPETS') . '</p>';
		$status = 'error';
		$success = false;

		if($user->authorise('snippet.pull', 'com_componentbuilder'))
		{
			// set success message
			$message = null;
			...
			// set redirect
			$redirect_url = Route::_('index.php?option=com_componentbuilder&view=pull_selection&power=Snippet&target=Snippets', false);
		}
		else
		{
			// set redirect
