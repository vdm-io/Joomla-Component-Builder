...
class PowerController extends FormController
{
	...
	public function resetPowers()
	{
		...
		{
			try {
				PowerFactory::_('Package.Builder.Get')->reset('power', [$guid]);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_RESET_OF_THIS_POWER_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_RESET_FAILED') . '</h1>';
	...
	public function pushPowers()
	{
		...
		{
			try {
				PowerFactory::_('Package.Builder.Set')->items('power', [$guid]);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_PUSH_OF_THIS_POWER_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PUSH_FAILED') . '</h1>';
