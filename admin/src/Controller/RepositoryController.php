...
class RepositoryController extends FormController
{
	...
	public function resetPowers()
	{
		...
		{
			try {
				RepositoryFactory::_('Package.Builder.Get')->reset('repository', [$guid]);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_RESET_OF_THIS_REPOSITORY_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_RESET_FAILED') . '</h1>';
	...
	public function pushPowers()
	{
		...
		{
			try {
				RepositoryFactory::_('Package.Builder.Set')->items('repository', [$guid]);
				...
				else
				{
					...
					$message .= '<p>' . Text::_('COM_COMPONENTBUILDER_THE_PUSH_OF_THIS_REPOSITORY_HAS_NOT_BEEN_SUCCESSFUL') . '</p>';
					$status = 'warning';
				}
			} catch (\Exception $e) {
				$message = '<h1>' . Text::_('COM_COMPONENTBUILDER_PUSH_FAILED') . '</h1>';
