...
class ComponentbuilderViewLibraries extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->targetOptions = FormHelper::loadFieldType('librariesfiltertarget')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_LIBRARY_TARGET_LABEL').' -',
				'batch[target]',
		...
		{
			// Set How Selection
			$this->howOptions = FormHelper::loadFieldType('Filebehaviour')->options;
			// We do some sanitation for How filter
			if (ArrayHelper::check($this->howOptions) &&
				isset($this->howOptions[0]->value) &&
				!StringHelper::check($this->howOptions[0]->value))
			{
				unset($this->howOptions[0]);
			}
			// How Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_LIBRARY_HOW_LABEL').' -',
				'batch[how]',
				Html::_('select.options', $this->howOptions, 'value', 'text')
			);
		}
		...
		{
			// Set Type Selection
			$this->typeOptions = FormHelper::loadFieldType('librariesfiltertype')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_LIBRARY_TYPE_LABEL').' -',
				'batch[type]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_COMPONENTBUILDER_LIBRARIES'));
		Html::_('stylesheet', "administrator/components/com_componentbuilder/assets/css/libraries.css", ['version' => 'auto']);
	}
