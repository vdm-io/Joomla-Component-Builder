...
class ComponentbuilderViewLanguage_translations extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		if ($this->canCreate)
		{
			ToolbarHelper::addNew('language_translation.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('language_translation.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('language_translations.publish');
				ToolbarHelper::unpublishList('language_translations.unpublish');
				ToolbarHelper::archiveList('language_translations.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('language_translations.checkin');
				}
			}
			...
			{
				ToolbarHelper::deleteList('', 'language_translations.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('language_translations.trash');
			}

			if ($this->canDo->get('core.export') && $this->canDo->get('language_translation.export'))
			{
				ToolbarHelper::custom('language_translations.exportData', 'download', '', 'COM_COMPONENTBUILDER_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('language_translation.import'))
		{
			ToolbarHelper::custom('language_translations.importData', 'upload', '', 'COM_COMPONENTBUILDER_IMPORT_DATA', false);
		}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_COMPONENTBUILDER_LANGUAGE_TRANSLATIONS'));
		Html::_('stylesheet', "administrator/components/com_componentbuilder/assets/css/language_translations.css", ['version' => 'auto']);
	}
