...
class ComponentbuilderViewTemplates extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		if (ArrayHelper::check($this->items))
		{
			...
			{
				ToolbarHelper::custom('templates.exportData', 'download', '', 'COM_COMPONENTBUILDER_EXPORT_DATA', true);
			}
		}
		...
		{
			ToolbarHelper::custom('templates.importData', 'upload', '', 'COM_COMPONENTBUILDER_IMPORT_DATA', false);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_TEMPLATE_DYNAMIC_GET_LABEL').' -',
				'batch[dynamic_get]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->add_php_viewOptions = FormHelper::loadFieldType('templatesfilteraddphpview')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_TEMPLATE_ADD_PHP_VIEW_LABEL').' -',
				'batch[add_php_view]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_COMPONENTBUILDER_TEMPLATES'));
		Html::_('stylesheet', "administrator/components/com_componentbuilder/assets/css/templates.css", ['version' => 'auto']);
	}
