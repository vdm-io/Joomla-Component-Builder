...
class ComponentbuilderViewCustom_admin_views extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		if (ArrayHelper::check($this->items))
		{
			...
			{
				ToolbarHelper::custom('custom_admin_views.exportData', 'download', '', 'COM_COMPONENTBUILDER_EXPORT_DATA', true);
			}
		}
		...
		{
			ToolbarHelper::custom('custom_admin_views.importData', 'upload', '', 'COM_COMPONENTBUILDER_IMPORT_DATA', false);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_CUSTOM_ADMIN_VIEW_MAIN_GET_LABEL').' -',
				'batch[main_get]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->add_php_ajaxOptions = FormHelper::loadFieldType('customadminviewsfilteraddphpajax')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_CUSTOM_ADMIN_VIEW_ADD_PHP_AJAX_LABEL').' -',
				'batch[add_php_ajax]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->add_custom_buttonOptions = FormHelper::loadFieldType('customadminviewsfilteraddcustombutton')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_CUSTOM_ADMIN_VIEW_ADD_CUSTOM_BUTTON_LABEL').' -',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_COMPONENTBUILDER_CUSTOM_ADMIN_VIEWS'));
		Html::_('stylesheet', "administrator/components/com_componentbuilder/assets/css/custom_admin_views.css", ['version' => 'auto']);
	}
