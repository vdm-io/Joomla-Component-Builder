...
class ComponentbuilderViewServers extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		if (ArrayHelper::check($this->items))
		{
			...
			{
				ToolbarHelper::custom('servers.exportData', 'download', '', 'COM_COMPONENTBUILDER_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('server.import'))
		{
			ToolbarHelper::custom('servers.importData', 'upload', '', 'COM_COMPONENTBUILDER_IMPORT_DATA', false);
		}
		...
		{
			// Set Name Selection
			$this->nameOptions = FormHelper::loadFieldType('serversfiltername')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_SERVER_NAME_LABEL').' -',
				'batch[name]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->protocolOptions = FormHelper::loadFieldType('serversfilterprotocol')->options;
			// We do some sanitation for Protocol filter
			if (ArrayHelper::check($this->protocolOptions) &&
				isset($this->protocolOptions[0]->value) &&
				!StringHelper::check($this->protocolOptions[0]->value))
			{
				unset($this->protocolOptions[0]);
			}
			// Protocol Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_COMPONENTBUILDER_SERVER_PROTOCOL_LABEL').' -',
				'batch[protocol]',
				Html::_('select.options', $this->protocolOptions, 'value', 'text')
			);
		}
	}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_COMPONENTBUILDER_SERVERS'));
		Html::_('stylesheet', "administrator/components/com_componentbuilder/assets/css/servers.css", ['version' => 'auto']);
	}
